...
public class JeuDeCartes {
	private Configuration[] typesDeCartes = { new Configuration(new Borne(25), 10),
			...
			new Configuration(new FinLimite(), 6), new Configuration(new Parade(Type.ESSENCE), 6), // Bidon d'essence
	...
	public String affichageJeuDeCartes() {
		...
		for (Configuration c : typesDeCartes) {
			sb.append(c.getNbExemplaires()).append(' ').append(c.getCarte().toString()).append('\n');
		}
		return sb.toString();
	}
	...
	public Carte[] donnerCartes() {
		...
		for (int i = 0, j = 0; i < typesDeCartes.length; i++) {
			for (int k = 0; k < typesDeCartes[i].getNbExemplaires(); k++) {
				toutes[j++] = typesDeCartes[i].getCarte();
			}
		}
		return toutes;
	}
...
//	public int nbExemplaireCarte(Configuration c) {
//		for (Carte)
//	}
//  faire la fonction auxiliere 
